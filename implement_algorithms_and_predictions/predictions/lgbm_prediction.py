...
def lgbm():
    ...
    for single_year in years:
        ...
        predicted_result.to_csv("final_prediction_csv/lgbm/predict_result_{}.csv".format(str(single_year)))
        
        predicted_result.plot(kind='barh',x='Movie',y='Predicted Win Rate',color='darkslateblue',figsize=(10, 5)) 
    
        plt.title("Light Gradient Boosting Machine Predictions") 
        plt.xlabel("Win rate") 
        plt.ylabel("") 
        plt.legend(['Predicted Win Rate'], loc = 'lower right')
        plt.tight_layout()
        plt.savefig('final_predictions_graphs/predictions/{}/{}_lgbm.png'.format(str(single_year),str(single_year)))
